...
class GPTNeoXModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        # Newer transformers (5.x) saves GPT-NeoX/Pythia rotary config as a nested
        # "rope_parameters" dict with "partial_rotary_factor" instead of the flat
        # "rotary_pct" key used by older versions.  Support both.
        rotary_pct = self.hparams.get("rotary_pct")
        if rotary_pct is None:
            rope_params = self.hparams.get("rope_parameters", {})
            rotary_pct = rope_params.get("partial_rotary_factor", 1.0)
        self.gguf_writer.add_rope_dimension_count(
            int(rotary_pct * (self.hparams["hidden_size"] // self.hparams["num_attention_heads"])),
        )
